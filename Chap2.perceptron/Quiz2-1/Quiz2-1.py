...
def show2D( vecW, X1, X2, ax ):
    ...
    # 分離境界線
    x_fig = np.array(np.arange(-2,3,1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--wvec', type=tp,    default=[1.0,-1.0], help='initial W vector')
    args = parser.parse_args()

    train_num = 6 # 学習データ数

    # class1の学習データ
    x1_1 = [-1.5, -1.0, -0.5]             # パターン1
    label_X1 = np.ones(train_num//2)      # ラベル1

    # class2の学習データ
    x2_1 = [-0.2,  0.2,  1.2]             # パターン2
    ...
    plt.savefig('result.Quiz2-1.png')
    #plt.savefig('result.Quiz2-1.eps')
    #plt.savefig('result.Quiz2-1.svg')
    plt.show() # 最後は表示をkeep
